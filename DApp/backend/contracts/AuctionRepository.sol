...
contract AuctionRepository{
     ...
     * @param _tokenRepositoryAddress address of the token repository to validate from
     * @param _tokenId uint256 ID of the token which has been registered in the token repository
     */
    modifier onlyTokenOwner(address _tokenRepositoryAddress, uint256 _tokenId) {
        address tokenOwner = TokenRepository(_tokenRepositoryAddress).ownerOf(_tokenId);
     ...
     */
    function getAuctionsAmount() public view returns(uint) {
    ...
    function createAuction(
        ...
        public onlyTokenOwner(_tokenRepositoryAddress, _tokenId) returns(bool) {
    ...
    function approveAndTransfer(address _from, address _to, address _tokenRepositoryAddress, uint256 _tokenId) internal returns(bool) {
        TokenRepository remoteContract = TokenRepository(_tokenRepositoryAddress);
     ...
     */
    function cancelAuction(uint _auctionId) public onlyAuctionOwner(_auctionId) {
        ...
        if(approveAndTransfer(address(this), myAuction.owner, myAuction.tokenRepositoryAddress, myAuction.tokenId)){
    ...
    function finalizeAuction(uint _auctionId) public {
        ...
        if(bidsLength == 0) {
            ...
            if(approveAndTransfer(address(this), lastBid.from, myAuction.tokenRepositoryAddress, myAuction.tokenId)){
                auctions[_auctionId].active = false;
